...
const groupOf = (file, status) => {
  if (file.startsWith("seatmaps/"))
    return status === 200 ? "seatMapCaptures" : "seatMapFailureCaptures";
...
for (const absolute of await walk(corpus)) {
  const file = relative(corpus, absolute).split(sep).join("/");
  if (file === "manifest.json" || !file.endsWith(".json")) continue;
  const { status } = JSON.parse(await readFile(absolute, "utf8"));
  members.get(groupOf(file, status)).push(file);
}

const files = [...members.values()].flat().sort();
const lines = [
  "import type {",
  "  Capture,",
...
for (const [name, body] of GROUPS) {
  ...
  for (const file of members.get(name).sort())
