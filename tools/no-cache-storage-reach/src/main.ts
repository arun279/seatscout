import { type Read, reaching, refusal, tracked } from "./reach.ts";
...
export const main = (
  ...
  listing: (pathspec: string) => string,
  read: Read,
  err: Writer,
): number => {
  const [pathspec = APPS] = argv.slice(2);
  const paths = tracked(listing(pathspec));
  if (paths.length === 0) {
    err.write(NOTHING);
    return 1;
  }

  const offenders = reaching(paths, read);
