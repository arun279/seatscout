...
The `footprint` job saves its incremental file whether it passes, fails or is cut short,
because Stryker writes what it has judged so far when it is interrupted, and a branch whose
run was cancelled would otherwise pay for that work twice.
