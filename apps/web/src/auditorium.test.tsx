...
describe("the room a result opens into", () => {
  ...
  it("gives every Seat focus, the not bookable and the accessible ones included, and refuses only their activation with a named reason", async () => {
    ...
    expect(small.verifications).toEqual([]);
  });
  ...
  it("carries a different Seat Group from the alternates into the hand-off the dock stacks over the room, and that candidate verifies", async () => {
    ...
    expect(within(sheet).getByText("G14·G13, yours")).toBeVisible();

    fireEvent.click(
      within(sheet).getByRole("button", { name: "Take G14 and G13" }),
    );
    const verified: Verified = await stage.answered();

    expect(stage.room.getByRole("grid")).toBeVisible();
    expect(verified.ok).toBe(true);
    expect(verified.ok && verified.result.seats.map((seat) => seat.id)).toEqual(
      ["G14", "G13"],
    );
    expect(verified.ok && verified.result.showtime.id).toBe(
      WEST_PLANO_28.showtime,
    );
